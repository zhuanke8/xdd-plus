...
!(async () => {
    ...
    console.log(`挖宝助力结束 成功助力${count}次`)
})()
    .catch((e) => {
        $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
    })
...
function gambleHomePage() {
    const headers = {
        ...
        'Referer': `https://bnzf.jd.com/?activityId=${activityId}&inviterId=${inviter}&inviterCode=${inviteCode}&utm_user=plusmember&ad_od=share&utm_source=androidapp&utm_medium=appshare&utm_campaign=t_335139774&utm_term=QQfriends`,
        'Accept-Language': 'zh-cn',
        'Cookie': cookie
    }
    const body = {"linkId": `${activityId}`, "inviter": `${inviter}`, "inviteCode": `${inviteCode}`};
    const options = {
        url: `https://api.m.jd.com/?functionId=happyDigHelp&body=${encodeURIComponent(JSON.stringify(body))}&appid=activities_platform&clientVersion=1.0.0h2&client=H5`,
        headers,
    }
