...
func getSign() *AutoGenerated {
	//data, err := httplib.Get("https://pan.smxy.xyz/sign").SetUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36").Bytes()
	data, err := httplib.Get("https://pan.smxy.xyz/sign").Bytes()
	for {
		...
		if err == nil {
			...
			logs.Info("重新获取sign")
			data, err = httplib.Get("https://hellodns.coding.net/p/sign/d/jsign/git/raw/master/sign").SetUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36").Bytes()
		}
	}
	//for {
	//	time.Sleep(2 * time.Second)
