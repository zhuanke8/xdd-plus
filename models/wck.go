...
func getSign() *AutoGenerated {
	data, _ := httplib.Get("https://api.jds.codes/jd/gentoken").SetUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36").Bytes()
	t := &AutoGenerated{}
	getString, _ := jsonparser.GetString(data, "data", "sign")
	json.Unmarshal([]byte(getString), t)
	i := 0
	for {
		time.Sleep(2 * time.Second)
		if t.Sign != "" {
...
//	t := &AutoGenerated{}
//	json.Unmarshal(data, t)
//	sprintf := "body=%7B%22to%22%3A%20%22https%3A//m.jd.com%22%2C%20%22action%22%3A%20%22to%22%7D" + fmt.Sprintf("&uuid=%s&client=apple&clientVersion=%s&st=%s&sv=%s&sign=%s", t.UUID, t.ClientVersion, t.St, t.Sv, t.Sign)
//	logs.Info(sprintf)
//	return sprintf
//}
