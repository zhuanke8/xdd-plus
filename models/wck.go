...
func genToken() string {
	...
	t := &AutoGenerated{}
	json.Unmarshal(data, t)
	sprintf := fmt.Sprintf("body=%7B%22to%22%3A%20%22https%3A//m.jd.com%22%2C%20%22action%22%3A%20%22to%22%7D&uuid=039d6e5e3e3e409a82dc1b1ce1110896&client=apple&clientVersion=9.4.0&st=%s&sv=%s&sign=%s", t.St, t.Sv, t.Sign)
	logs.Info(sprintf)
	return sprintf
}
...
func getOKKey1(WSCK string) (string, error) {
	s := genToken()
	random := browser.Random()
	req := httplib.Post(`https://api.m.jd.com/client.action?` + s + "&clientVersion=10.3.5&functionId=genToken")
	req.Header("cookie", WSCK)
	req.Header("User-Agent", random)
	req.Header("content-type", `application/x-www-form-urlencoded; charset=UTF-8`)
	req.Header("charset", `UTF-8`)
